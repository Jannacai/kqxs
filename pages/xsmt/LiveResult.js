...
const LiveResult = React.memo(
    ({ getHeadAndTailNumbers = () => { }, handleFilterChange = () => { }, filterTypes = {}, isLiveWindow = true }) => {
        ...
        useEffect(() => {
            ...
            if (
                ...
                console.log(`Reusing session ID: ${sessionData.sessionId}`);
            }
        ...
        const renderPrizeValue = useMemo(() => {
            return (tinh, prizeType, digits = 5) => {
                ...
                const filterType = filterTypes[filterKey] || 'all'; // Giá trị mặc định
                ...
                console.log(
                    `Rendering prize for ${tinh}, ${prizeType}: isAnimating=${isAnimating}, prizeValue=${prizeValue}, filterType=${filterType}, filterKey=${filterKey}`
                );
