    ...
    const fetchSoiCauData = useCallback(async (date, days) => {
        ...
        try {
            ...
            if (response.metadata?.message && response.predictions.length === 0) {
                setError(response.metadata.message || `Không có dữ liệu soi cầu cho tỉnh ${tinh} vào ngày ${targetDate}. Vui lòng chọn tỉnh hoặc ngày khác.`);
            }
        } catch (err) {
        ...
        <div className={styles.container}>
            ...
            <div className={styles.content}>
                ...
                <div className={styles.groupSelect}>
                    <label className={styles.options}>Số ngày phân tích:</label>
                    ...
                    <label className={styles.options}>Tỉnh:</label>
                ...
                {!loadingLottery && !error && lotteryData.length > 0 && !isBeforeResultTime && (
                    <>
                        <h2 className={styles.heading}>Kết quả xổ số ngày {metadata.dataTo || 'Chưa có dữ liệu'} (dùng để soi cầu)</h2>
                        <table className={styles.lotteryTable}>
                            <thead>
                                <tr>
                                    <th>ĐB</th>
                                                                            ...
                                                                            : result.method === 'Tổng các chữ số'
                                                                                ...
                                                                                : result.method === 'Đảo ngược'
                                                                                    ...
                                                                                    : result.method === 'Khoảng cách'
                                                                                        ...
                                                                                        : result.method === 'Xu hướng gần đây'
                ...
                <div className={styles.relatedNumbers}>
                    <div>Lô kép: <strong>{relatedNumbers.loKep.length > 0 ? relatedNumbers.loKep.join(', ') : 'Không có'}</strong></div>
                    <div>Lô gan: <strong>{relatedNumbers.loGan.length > 0 ? relatedNumbers.loGan.slice(0, 10).join(', ') : 'Không có'}</strong></div>
                </div>
                                    ...
                                    <td>
                                        {item.actualNumbers?.length > 0
                                            ? item.actualNumbers.map((num, i) => (
                                                <span key={i} className={styles.matchedNumber}>{num}</span>
                                            ))
                                            : 'Không có'}
                                    </td>
